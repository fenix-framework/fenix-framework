 ...
 * associated with a key obtained by invoking {@link pt.ist.fenixframework.backend.BackEnd#getName()} on the config's BackEnd.
 * 
 ...
 * 
 * For this mechanism to work as expected when using explicit initialization (see {@link FenixFramework}), the programmer must
 * initialize the framework by invoking the {@link FenixFramework#initialize(MultiConfig)} method (instead of
 * {@link FenixFramework#initialize(Config)}). Before doing so, however, the programmer must create an
