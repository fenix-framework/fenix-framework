...
public class InnerNodeArray extends InnerNodeArray_Base {
    ...
    private DoubleArray<AbstractNodeArray> justInsert(Comparable middleKey, AbstractNodeArray subLeftNode,
            AbstractNodeArray subRightNode) {
        DoubleArray<AbstractNodeArray> newArr = getSubNodes().duplicateAndAddKey(middleKey, subLeftNode, subRightNode);
        setSubNodes(newArr);
        return newArr;
    }

    private DoubleArray<AbstractNodeArray> justInsertUpdatingParentRelation(Comparable middleKey, AbstractNodeArray subLeftNode,
            AbstractNodeArray subRightNode) {
    ...
    AbstractNodeArray underflowFromLeaf(Comparable deletedKey, Comparable replacementKey) {
        ...
        if (iter == 0) { // the deletedKey was removed from the first sub-node
            ...
            if (iter >= (subNodes.length() - 1)
                    || (nextEntryValue = subNodes.values[iter + 1]).shallowSize() > BPlusTreeArray.LOWER_BOUND) { // caution: tricky test!!
    ...
    private void moveChildFromLeftToRight(Comparable leftEntryKey, AbstractNodeArray leftEntryValue,
            AbstractNodeArray rightEntryValue) {
    ...
    AbstractNodeArray underflowFromInner(InnerNodeArray deletedNode) {
        ...
        if (iter == 0) { // the deletion occurred in the first sub-node
            ...
            if (iter >= (subNodes.length() - 1)
                    || (nextEntryValue = subNodes.values[iter + 1]).shallowSize() > BPlusTreeArray.LOWER_BOUND_WITH_LAST_KEY) { // caution: tricky test!!
                // either there is no next or the next is above the lower bound
