...
public abstract class AbstractNodeArrayGhost<T extends Serializable> extends AbstractNodeArrayGhost_Base implements Iterable {
    /* Node Interface */

    /** Inserts the given key-value pair and returns the (possibly new) root node */
    abstract AbstractNodeArrayGhost insert(Comparable key, T value);

    /** Removes the element with the given key */
    abstract AbstractNodeArrayGhost remove(Comparable key);
     ...
     * @throws IndexOutOfBoundsException if the index is out of range (index < 0 || index >= size())
     */
    abstract AbstractNodeArrayGhost removeIndex(int index);
    ...
    public AbstractNodeArrayGhost() {
        this((LocalityHints) null);
    }

    public AbstractNodeArrayGhost(eu.cloudtm.LocalityHints hints) {
        super(hints);
    }

    AbstractNodeArrayGhost getRoot() {
        InnerNodeArrayGhost thisParent = this.getParent();
        return thisParent == null ? this : thisParent.getRoot();
    }
    
    AbstractNodeArrayGhost getRootGhost() {
	InnerNodeArrayGhost thisParent = this.getParentGhost();
	return thisParent == null ? this : thisParent.getRootGhost();
    }
    ...
    abstract void mergeWithLeftNode(AbstractNodeArrayGhost leftNode, Comparable splitKey);
