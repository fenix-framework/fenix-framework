...
public abstract class AbstractNodeGhost<T extends Serializable> extends AbstractNodeGhost_Base implements Iterable {
    /* Node Interface */

    /** Inserts the given key-value pair and returns the (possibly new) root node */
    abstract AbstractNodeGhost insert(Comparable key, T value);

    /** Removes the element with the given key */
    abstract AbstractNodeGhost remove(Comparable key);
     ...
     * @throws IndexOutOfBoundsException if the index is out of range (index < 0 || index >= size())
     */
    abstract AbstractNodeGhost removeIndex(int index);
    ...
    public AbstractNodeGhost() {
        this((LocalityHints) null);
    }

    public AbstractNodeGhost(LocalityHints hints) {
        super(hints);
    }

    AbstractNodeGhost getRoot() {
        InnerNodeGhost thisParent = this.getParent();
        return thisParent == null ? this : thisParent.getRoot();
    }
    
    AbstractNodeGhost getRootGhost() {
	InnerNodeGhost thisParent = this.getParentGhost();
    ...
    abstract void mergeWithLeftNode(AbstractNodeGhost leftNode, Comparable splitKey);
