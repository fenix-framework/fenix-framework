...
public class LeafNodeGhost extends LeafNodeGhost_Base {

    public LeafNodeGhost(LocalityHints hints) {
	super(hints);
	setEntries(new TreeMap<Comparable,Serializable>(BPlusTree.COMPARATOR_SUPPORTING_LAST_KEY));
    }

    private LeafNodeGhost(LocalityHints hints, TreeMap<Comparable,Serializable> entries) {
	super(hints);
	setEntries(entries);
    }
	...
	if (localMap.size() <= BPlusTree.MAX_NUMBER_OF_ELEMENTS) { // it still fits :-)
	    ...
	    // split node in two
	    LeafNodeGhost leftNode = new LeafNodeGhost(this.getLocalityHints(), new TreeMap<Comparable,Serializable>(localMap.headMap(keyToSplit)));
	    LeafNodeGhost rightNode = new LeafNodeGhost(this.getLocalityHints(), new TreeMap<Comparable,Serializable>(localMap.tailMap(keyToSplit)));
	    fixLeafNodeGhostsListAfterSplit(leftNode, rightNode);
	...
	LeafNodeGhost left = (LeafNodeGhost)leftNode; // this node does not know how to merge with another kind

	TreeMap<Comparable,Serializable> newMap = duplicateMap();
	newMap.putAll(left.getEntries());
	setEntries(newMap);

	LeafNodeGhost nodeBefore = left.getPrevious();
	...
	if (index < shallowSize()) { // the required position is here
	    ...
	    LeafNodeGhost next = this.getNext();
	    if (next == null) {
		throw new IndexOutOfBoundsException();
	    }
	...
	return new LeafNodeGhostKeysIterator(this);
    }
    ...
    private class LeafNodeGhostValuesIterator extends GenericLeafNodeGhostIterator<Serializable> {

	LeafNodeGhostValuesIterator(LeafNodeGhost leafNode) {
	    super(leafNode);
	}
    ...
    private class LeafNodeGhostKeysIterator extends GenericLeafNodeGhostIterator<Comparable> {

	LeafNodeGhostKeysIterator(LeafNodeGhost leafNode) {
	    super(leafNode);
	}

	protected Iterator<Comparable> getInternalIterator(LeafNodeGhost leafNode) {
