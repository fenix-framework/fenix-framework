 ...
 * comparable to each other (e.g. the same BPlusTreeGhost instance cannot simultaneously support keys of
 * type Integer and String).
 */
public class BPlusTreeGhost<T extends Serializable> extends BPlusTreeGhost_Base implements DomainBasedMap<T> {

    public static final String LOWER_BOUND_PROPERTY = "btreeLowerBound";
    public static final String DEFAULT_LOWER_BOUND = "100";
    private static final Logger logger = LoggerFactory.getLogger(BPlusTreeGhost.class);
    ...
    public BPlusTreeGhost() {
        initRoot();
    }
    
    public BPlusTreeGhost(LocalityHints hints) {
	super(hints);
	initRoot();
    }

    private void initRoot() {
        this.setRoot(new LeafNodeGhost(this.getLocalityHints()));
    }
    ...
    public boolean insert(Comparable key, T value) {
    	try {
    		...
    		AbstractNodeGhost rootNode = this.getRoot();
    		AbstractNodeGhost resultNode = rootNode.insert(key, value);
    ...
    public boolean removeKey(Comparable key) {
    	try {
    		AbstractNodeGhost rootNode = this.getRoot();
    		AbstractNodeGhost resultNode = rootNode.remove(key);
    ...
    public T getIndex(int index) {
        return ((AbstractNodeGhost<T>) this.getRoot()).getIndex(index);
    }
    ...
    public T removeIndex(int index) {
        ...
        AbstractNodeGhost rootNode = this.getRoot();
        AbstractNodeGhost resultNode = rootNode.removeIndex(index);
        if (rootNode != resultNode) {
            this.setRoot(resultNode);
        }
