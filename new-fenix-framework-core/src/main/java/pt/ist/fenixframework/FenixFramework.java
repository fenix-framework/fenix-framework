...
/**
 * <p>This is the main class for the Fenix Framework.  It is the central point
 * for obtaining most of the APIs that the user of the framework (a programmer)
 * should need.
 *
 * <p>Before being able to use the framework, a program must initialize it, by
 * providing a configuration.  There are two (disjoint) alternative methods to
 * initialize the FenixFramework:
 * 
 * <ul>
 *
 * <li>Explicitly: The programmer creates the {@link Config} instance and
 * explicitly calls the {@link FenixFramework#initialize(Config)} method.</li>
 *
 * <li>By convention: When the FenixFramework class is loaded, its static
 * initialization code will check for the presence of the file
 * 'fenix-framework.properties' in the classpath using
 * <code>Thread.currentThread().getContextClassLoader().getResource("fenix-framework.properties")</code>.
 * If this file is found, and contains the correct content, the framework will
 * create the appropriate Config (or subclass) instance and automatically invoke
 * {@link FenixFramework#initialize(Config)}.</li>
 *
 * </ul>
 * 
 * <p>The first alternative maintains the previous configuration style with
 * compile-time checking of the config's attributes. To use it, read the
 * documentation in the {@link Config} class.
 *
 * <p>Using the second option, the syntax for the configuration file is to have
 ...
 * </blockquote>
 *
 * where each <code>property</code> must be the name of an existing
 * configuration field.  Additionally, there is one optional property named
 * <code>config.class</code>.  By default, the config parser creates an instance
 * of {@link pt.ist.fenixframework.core.DefaultConfig}, unless the
 * <code>config.class</code> is given, in which case the mentioned class'
 * default constructor is invoked to create an instance of a more specific
 * Config instance.  The config instance is then populated with each property
 * (except with the <code>config.class</code> property), using the following
 * algorythm:
 *
 * <ol>
 *
 * <li> Confirm that the config has a field with the same name as the
 * <code>property</code>.  If not a {@link ConfigError} is thrown.</li>
 *
 * <li> If the config class provides a method (can be private) in the format
 * <code>&lt;property&gt;FromString(String)</code>, then such method will be
 * used to set the property.  The String argument will be the
 * <code>value</code>.</li>
 *
 * <li> Else, attempt to directly set the property on the field assigning it the
 * <code>value</code> String.</li>
 *
 * <li> If all previous attempts have failed, throw a {@link ConfigError}.</li>
 *
 * </ol>
 *
 * <p>The rationale supporting the previous mechanism is to allow the config
 * class to process the String provided in the <code>value></code> using the
 * <code>*FromString</code> method.
 * 
 * <p> After population of the config finishes with success, the {@link
 * FenixFramework#initialize(Config)} method is invoked with the created Config
 * instance.
 *
 * <p>After initialization, it is possible to get an instance of the {@link
