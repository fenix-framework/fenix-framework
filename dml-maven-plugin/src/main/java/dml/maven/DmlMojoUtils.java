...
public class DmlMojoUtils {
	public static FenixFrameworkArtifact getArtifact(MavenProject project,
			File srcDirectoryFile, File generatedSourcesDirectoryFile,
			List<URL> dmlFiles, Log log, boolean verbose) throws IOException,
		...
		for (Artifact artifact : project.getDependencyArtifacts()) {
			...
			if (file.lastModified() > generatedSourcesDirectoryFile
					.lastModified()) {
				if (verbose) {
					log.info("Dependency "
			...
			if (jarFile.getJarEntry(artifact.getArtifactId()
					+ "/project.properties") != null) {
				dependencies.add(FenixFrameworkArtifact.fromName(artifact
						.getArtifactId()));
			}
			jarFile.close();
		}
		...
		return new FenixFrameworkArtifact(project.getArtifactId(), dmls,
