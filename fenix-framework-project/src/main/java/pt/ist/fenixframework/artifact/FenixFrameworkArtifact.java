...
public class FenixFrameworkArtifact {
	...
	public FenixFrameworkArtifact(String name, List<DmlFile> dmls,
			List<FenixFrameworkArtifact> dependencies, boolean shouldCompile)
	...
	public List<FenixFrameworkArtifact> getArtifacts() {
		...
		while (!freeNodes.isEmpty()) {
			...
			for (FenixFrameworkArtifact dependency : artifact
	...
	private static void computeIncomingEdges(
			Map<FenixFrameworkArtifact, List<FenixFrameworkArtifact>> incoming,
			FenixFrameworkArtifact artifact) {
		incoming.put(artifact, new ArrayList<FenixFrameworkArtifact>(
				artifact.depended));
		for (FenixFrameworkArtifact dependency : artifact
				.getDependencyArtifacts()) {
	...
	public void generateProjectProperties(String outputDirectory)
		...
		properties.setProperty(DML_FILES_KEY,
				StringUtils.join(getDmls(), SEPARATOR_CHAR));
		if (dependencies.size() > 0) {
			properties.setProperty(DEPENDS_KEY,
					StringUtils.join(getDependencyArtifacts(), SEPARATOR_CHAR));
		}
		File output = new File(outputDirectory + "/" + getName()
				+ "/project.properties");
		output.getParentFile().mkdirs();
		properties.store(new FileWriter(output), null);
	}

	public static FenixFrameworkArtifact fromName(String artifactName)
			throws IOException, FenixFrameworkProjectException,
			MalformedURLException {
		Properties properties = new Properties();
		properties.load(Thread.currentThread().getContextClassLoader()
				.getResourceAsStream(artifactName + "/project.properties"));
		return FenixFrameworkArtifact.fromProperties(properties);
	}

	public static FenixFrameworkArtifact fromProperties(Properties properties)
			throws MalformedURLException, IOException,
			FenixFrameworkProjectException {
		String name = properties.getProperty(NAME_KEY);
		if (!artifacts.containsKey(name)) {
			List<DmlFile> dependencyDmlFiles = DmlFile
					.parseDependencyDmlFiles(properties
							.getProperty(DML_FILES_KEY));
			List<FenixFrameworkArtifact> dependencies = new ArrayList<FenixFrameworkArtifact>();
			for (String artifactName : properties.getProperty(DEPENDS_KEY, "")
					.trim().split("\\s*,\\s*")) {
				if (StringUtils.isNotEmpty(artifactName)) {
					dependencies.add(FenixFrameworkArtifact
							.fromName(artifactName));
				}
			}
